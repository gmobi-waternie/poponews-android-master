...
public class GoogleService implements IGoogleService {
	...
	private GoogleNewsParser parser = new GoogleNewsParser();
	private GoogleApiClient mGoogleApiClient;

	private final String SERVICE_TYPE = SocialExtra.SOCIAL_TYPE_GOOGLE;
	@Override
	...
	public void remoteGetAccountList(int fromActivity) {
		...
		for(int i=0; i<se.getSources().size(); i++)
		{
			if(se.getSources().get(i).getName().equals(SERVICE_TYPE))
			{
				...
				for(int j=0; j<configs.getChannels().size(); j++)
				{
					...
					acc.setRole(SocialAccount.ROLE_GOOGLE_CHANNEL);
					acc.setExtra("");
		...
		if(fromActivity == ISocialService.FROM_MAIN)
			messageService.send(this, MessageTopics.GET_GOOGLE_CHANNEL_MAIN);
		else
			messageService.send(this, MessageTopics.GET_GOOGLE_CHANNEL_SETTING);
	}
	...
	public void doLogin(Object extra, int from) {
		...
		activity.startActivityForResult(signInIntent, GlobalConfig.GOOGLE_SIGN_IN);
//		if (from == ISocialService.FROM_LOGIN){
//		} else if (from == ISocialService.FROM_BIND){
//			messageService.send(this,MessageTopics.GET_GOOGLE_ME_BIND,SocialExtra.SOCIAL_TYPE_GOOGLE);
//		}

		socialDataService.ShowTypeInAccList(SERVICE_TYPE);
		socialDataService.ShowTypeInPostList(SERVICE_TYPE);

		AnalysisUtil.recordSnsLogin(SocialExtra.SOCIAL_TYPE_GOOGLE, AnalysisUtil.RESULT_SUCCESS);
	...
	public void doLogout(Object extra) {
		...
		AnalysisUtil.recordSnsLogin(SocialExtra.SOCIAL_TYPE_GOOGLE, AnalysisUtil.RESULT_LOGOUT);
	...
	private void downloadChannelData(final SocialAccount sa,String uri)
	{
		...
		Logger.debug("Start download google channel:"+uri);
		...
		rssSession.getStateChangedEvent().addEventHandler(new IEventHandler<HttpSession.StateChangedEventArgs>() {
			...
			public void process(Object sender,
				...
				if (args.getState() == HttpSession.STATE_FINISHED) {
					...
					try {
						DataList<SocialPost> googlenewsList = parser.parse(self.getFile());
						socialDataService.addIntoPostList(googlenewsList);
						sa.setFetchStatus(SocialAccount.STATUS_IDLE);
						Log.e("Google", "Google sa=" + sa.getName());

						AnalysisUtil.recordSnsFetch(SocialExtra.SOCIAL_TYPE_GOOGLE, googlenewsList.getItemCount() + "");
						messageService.send(this, MessageTopics.GET_GOOGLE_POST);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
